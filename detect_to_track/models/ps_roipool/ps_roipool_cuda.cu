...
)
{
    ...
    )
    {
        ...
        const scalar_t cI(rI - rH/2 + (static_cast<scalar_t>(i) + 0.5) * cH);  // of cell c
        const scalar_t cJ(rJ - rW/2 + (static_cast<scalar_t>(j) + 0.5) * cW);  // of cell c
        ...
        const int pooledNumel((cI1 - cI0) * (cJ1 - cJ0));
        if (pooledNumel > 0)
            out[ind] /= pooledNumel;
    }
}
    ...
    )
    {
        ...
        for (int pI = cI0; pI < cI1; ++pI) {
            for (int pJ = cJ0; pJ < cJ1; ++pJ) {
                ...
                scalar_t addVal(gradOut[ind]);
                if (roiNumel > 0)
                    addVal /= roiNumel;
                atomicAdd(gradInChannel + (pI * iW + pJ), addVal);
            }
        }
    }
}
